...
def parse_name(fname):
    cond = re.search(r"(?:^|[_ .])c(?:ond)?[_ ]?(\d+)", fname, re.IGNORECASE)
    day = re.search(r"(?:^|[_ .])d(?:ay)?[_ ]?(\d+)", fname, re.IGNORECASE)
...
def main(in_dir, out_dir):
    ...
    for f in sorted(in_dir.glob("*.tif*")):
        ...
        summary.append({
            "image": base, "condition": cond, "day": day,
        ...
        d.insert(0, "image", base)
        d.insert(1, "condition", cond)
        d.insert(2, "day", day)
        droplets.append(d)
        print(f"{base}: {cond} {day} n={labels.max()} "
